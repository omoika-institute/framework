...
`Settings -> Global Settings -> Plugin Settings`. The UI stores overrides by
normalized transform label, for example `Run Sherlock` becomes `run-sherlock`,
and also supports shared values under `__global__`. When Electron runs a
transform, it merges `__global__` and the selected transform's saved values into
the top level of `cfg`, so transform code can read values directly with
`cfg.get("key")`.
