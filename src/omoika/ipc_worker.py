...
class ObWorker:
    def __init__(self) -> None:
        self.plugins_path_key: str | None = None

    def _normalize_plugins_paths(
        self, plugins_path: str | list[str] | tuple[str, ...] | None = None
    ) -> list[str]:
        if plugins_path is None:
            return [_default_plugins_path()]

        raw_values: list[Any]
        if isinstance(plugins_path, (list, tuple, set)):
            raw_values = list(plugins_path)
        else:
            raw_values = [plugins_path]
        ...
        return normalized or [_default_plugins_path()]
    ...
    def ensure_plugins(
        ...
        paths = self._normalize_plugins_paths(plugins_path)
        key = json.dumps(paths)
        if self.plugins_path_key == key:
            return
        self._reset_registry()
        for path in paths:
            load_plugins_fs(path)
        self.plugins_path_key = key
    ...
    def list_entities(
        ...
        for plugin in Registry.plugins.values():
            ...
            with open(path, "r") as fh:
                source = fh.read()
            last_file_edit = datetime.fromtimestamp(
    ...
    async def list_transforms(
        self,
        label: str,
        plugins_path: str | list[str] | tuple[str, ...] | None = None,
    ) -> list[dict[str, Any]]:
        self.ensure_plugins(plugins_path)
        if not label:
            return []
    ...
    async def run_transform(
        ...
        if "entity" in src:
            ...
            source_entity_label = (
                ...
                or entity_payload.get("data", {}).get("label")
            )
        else:
        ...
        entity_arg = TransformPayload(
            **{
                **{to_snake_case(k): v for k, v in entity_data.items()},
                ...
                "label": entity_data.get("label") or source_entity_label,
            }
        )
        ...
        try:
            ...
            raise build_transform_error(exc, transform_fn) from exc
...
async def _send_transform_events(
    ...
    worker: ObWorker,
    source: str | dict[str, Any],
    plugins_path: str | list[str] | tuple[str, ...] | None,
